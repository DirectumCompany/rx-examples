...
=======
	public class ModuleFunctions
	{
		...
		public static Sungero.Parties.ICounterparty GetCounterparty(List<ArioExtensions.Models.Fact> facts)
		{
			...
			var foundByName = new List<ICounterparty>();
			foundByName = Counterparties.GetAll().Where(x => correspondentNames.Any(n => n == x.Name) &&
			                                            x.Status != Sungero.CoreEntities.DatabookEntry.Status.Closed &&
			                                            (x.Note == null || !x.Note.Equals(BusinessUnits.Resources.BusinessUnitComment))).ToList();
				...
				{							
					...
					if (!foundByTin.Any())
					{
						foundByName = foundByName.Where(x => string.IsNullOrEmpty(x.TIN) &&
						                                (!(x is ICompanyBase) || string.IsNullOrEmpty((x as ICompanyBase).TRRC))).ToList();
						return foundByName.FirstOrDefault();
					}
					else
					{
		...
		/// Получить значение поля из фактов.
		/// </summary>
		/// <param name="fact">Факт, поле которого будет извлечено.</param>
		/// <param name="fieldName">Имя поля, значение которого нужно извлечь.</param>
		/// <returns>Зачение поля.</returns>
		public static string GetField(ArioExtensions.Models.Fact fact, string fieldName)
		{
		...
		public static string GetField(List<ArioExtensions.Models.Fact> facts, string factName, string fieldName)
		{
			...
			var field = fields.FirstOrDefault(f => f.Name.Equals(fieldName, StringComparison.InvariantCultureIgnoreCase));
			if (field != null)
				return field.Value;
			return string.Empty;
		}
		...
		public static List<ICounterparty> GetCounterparties(string tin, string trrc)
		{
			...
			// Отфильтровать закрытые сущности и копии НОР.
			counterparties = counterparties.Where(x => x.Status != Sungero.CoreEntities.DatabookEntry.Status.Closed &&
			                                      (x.Note == null || !x.Note.Equals(BusinessUnits.Resources.BusinessUnitComment)));
			...
			{
				var counterpartiesByTin = counterparties.Where(x => x.TIN == tin);
				count = counterpartiesByTin.Count();
				
				// Поиск по КПП, если КПП передан.
				if (searchByTrrc)
				{
					var companies = counterpartiesByTin.ToList().Where(x => CompanyBases.Is(x)).Select(x => CompanyBases.As(x)).ToList();
...
>>>>>>> #82088 Переписала алгоритм поиска.
}